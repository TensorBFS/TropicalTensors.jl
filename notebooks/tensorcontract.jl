...
f = HDF5.h5open(TropicalTensors.("benchmarks", "ising.hdf5"), "r")
res = read(f)
instance = res["n120"]["seed97"]
...
cub = CuArray(b);
@benchmark a * b
@benchmark CUDA.@sync cua * cub

a = zeros(CountingTropical{Float64}, 1000, 1000);
b = zeros(CountingTropical{Float64}, 1000, 1000);
cua = CuArray(a);
cub = CuArray(b);
@benchmark a * b
@benchmark CUDA.@sync cua * cub
